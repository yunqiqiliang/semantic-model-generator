...
def qualify_sql_with_semantic_context(
    sql: str,
    table_mapping: Dict[str, Tuple[str, str, str]],
) -> str:
    """
    Rewrite SQL so table references use fully-qualified identifiers based on the semantic model context.

    Args:
        sql: Raw SQL string (potentially generated by the LLM).
        table_mapping: Mapping of semantic table name (upper-cased) to (catalog, schema, table).

    Returns:
        SQL string with fully-qualified table references when matches are found.
    """
    ...
    def _update_table(node: sqlglot.exp.Expression) -> bool:
        if isinstance(node, sqlglot.exp.Table):
            ...
            if isinstance(identifier, sqlglot.exp.Identifier):
                table_name = identifier.this.upper()
                if table_name in table_mapping:
                    catalog, schema, table = table_mapping[table_name]
                    node.set("catalog", sqlglot.exp.Identifier(this=catalog))
                    node.set("db", sqlglot.exp.Identifier(this=schema))
                    node.set("this", sqlglot.exp.Identifier(this=table))
        return True

    expression.walk(_update_table)
    return expression.sql(dialect=ClickzettaDialect, pretty=False)
...
def validate_yaml_content(content: str) -> bool:
    ...
    try:
        ...
        st.session_state.pop("join_dialog_open", None)
    ...
    for item in content:
        if item["type"] == "text":
            ...
            with st.container(height=500, border=False):
                ...
                table_mapping: Dict[str, Tuple[str, str, str]] = {}
                if hasattr(st.session_state, "semantic_model"):
                    semantic_model = st.session_state.semantic_model
                    if getattr(semantic_model, "tables", None):
                        for table in semantic_model.tables:
                            base = getattr(table, "base_table", None)
                            if (
                                base
                                and getattr(base, "database", None)
                                and getattr(base, "schema", None)
                                and getattr(base, "table", None)
                            ):
                                table_mapping[table.name.upper()] = (
                                    base.database,
                                    base.schema,
                                    base.table,
                                )

                qualified_sql = qualify_sql_with_semantic_context(sql, table_mapping)
                pretty_sql = pretty_print_sql(qualified_sql)
                with st.container(height=250, border=False):
                    st.code(pretty_sql, language="sql")

                try:
                    if hasattr(conn, "session") and hasattr(conn.session, "sql"):
                        df = conn.session.sql(qualified_sql).to_pandas()
                    else:
                        df = pd.read_sql(qualified_sql, conn)
                except Exception as exc:
                    logger.error("Failed to execute SQL query: {}", exc)
                    st.error(f"Unable to execute SQL: {exc}")
                    return
